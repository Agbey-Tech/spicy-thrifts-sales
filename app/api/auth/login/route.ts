...
import { signInSchema } from "@/core/validation/auth.schema";
...
export const POST = withErrorHandling(async (req: NextRequest) => {
  ...
  // You may want to add a signInSchema for validation
  const input = validate(signInSchema, body);
  const created = await service.login(input);
